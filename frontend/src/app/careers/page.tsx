...
export async function generateMetadata({ searchParams }: { searchParams: { locale?: string } }): Promise<Metadata> {
  const content = await fetchContent('careers', searchParams.locale)
  if (!content) return { title: 'Careers' }
  return {
...
export default async function CareersPage({ searchParams }: { searchParams: { locale?: string } }) {
  const content = await fetchContent('careers', searchParams.locale)
  if (!content || !content.body) return <ComingSoon title="Careers" />
