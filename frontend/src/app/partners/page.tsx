...
export async function generateMetadata({ searchParams }: { searchParams: { locale?: string } }): Promise<Metadata> {
  const content = await fetchContent('partners', searchParams.locale)
  if (!content) return { title: 'Our Partners' }
  return {
...
export default async function PartnersPage({ searchParams }: { searchParams: { locale?: string } }) {
  const content = await fetchContent('partners', searchParams.locale)
  if (!content || !content.body) return <ComingSoon title="Our Partners" />
      ...
      <section className="bg-gradient-to-br from-cyan-700 via-cyan-800 to-blue-900 py-20 md:py-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Breadcrumb
            items={[
              { label: 'Home', href: '/' },
              { label: content.title || 'Our Partners' },
            ]}
            className="mb-6"
          />
