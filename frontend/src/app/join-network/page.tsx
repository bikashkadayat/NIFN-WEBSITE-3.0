...
export async function generateMetadata(): Promise<Metadata> {
  const content = await fetchContent('join-network')
  if (!content) return { title: 'Join Network | NIFN' }
  return {
...
export default async function JoinNetworkPage() {
  const content = await fetchContent('join-network')
  if (!content || !content.body) return <ComingSoon title="Join Network" />

  return (
    <>
      <section className="bg-gradient-to-br from-cyan-700 via-cyan-800 to-blue-900 py-20 md:py-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Breadcrumb items={[{ label: 'Home', href: '/' }, { label: content.title || 'Join Network' }]} className="mb-6" />
