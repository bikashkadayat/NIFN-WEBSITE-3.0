...
export async function generateMetadata({ searchParams }: { searchParams: { locale?: string } }): Promise<Metadata> {
  const content = await fetchContent('cookie-policy', searchParams.locale)
  if (!content) return { title: 'Cookie Policy' }
  return {
...
export default async function CookiePolicyPage({ searchParams }: { searchParams: { locale?: string } }) {
  const content = await fetchContent('cookie-policy', searchParams.locale)
  if (!content || !content.body) return <ComingSoon title="Cookie Policy" />
      ...
      <section className="bg-gradient-to-br from-cyan-700 via-cyan-800 to-blue-900 py-20 md:py-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Breadcrumb
            items={[
              { label: 'Home', href: '/' },
              { label: content.title || 'Cookie Policy' },
            ]}
            className="mb-6"
          />
