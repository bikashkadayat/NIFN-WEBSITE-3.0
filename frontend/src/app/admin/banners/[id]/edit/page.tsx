...
import { useBanner, useUpdateBanner } from '@/hooks/use-queries'
...
export default function EditBannerPage() {
  ...
  if (banner && !loaded) {
    const enT = banner.translations?.find(t => t.locale === 'en')
    const neT = banner.translations?.find(t => t.locale === 'ne')
    setEn({ title: enT?.title || '', subtitle: enT?.subtitle || '', primary_button_text: enT?.primary_button_text || '', secondary_button_text: enT?.secondary_button_text || '' })
    setNe({ title: neT?.title || '', subtitle: neT?.subtitle || '', primary_button_text: neT?.primary_button_text || '', secondary_button_text: neT?.secondary_button_text || '' })
    setImageId(banner.image_id || '')
    setImageUrl(banner.image?.url || '')
    setTextAlignment(banner.text_alignment || 'center')
    setOverlayOpacity(banner.overlay_opacity ?? 50)
    setPrimaryLink(banner.primary_button_link || '')
    setSecondaryLink(banner.secondary_button_link || '')
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <ImageUploader value={imageUrl} onChange={(id, url) => { setImageId(id); setImageUrl(url) }} />
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" value={primaryLink} onChange={(e) => setPrimaryLink(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <input type="text" value={secondaryLink} onChange={(e) => setSecondaryLink(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button type="submit" disabled={updateMutation.isPending} className="px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 text-sm font-medium">
