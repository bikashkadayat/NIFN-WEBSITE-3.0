...
import { useContent, useUpdateContent } from '@/hooks/use-queries'
...
export default function EditContentPage() {
  ...
  if (content && !loaded) {
    const enTrans = content.translations?.find(t => t.locale === 'en')
    const neTrans = content.translations?.find(t => t.locale === 'ne')
    setTranslations({
      en: {
        title: enTrans?.title || '',
        body: enTrans?.body || '',
    ...
    setFeaturedImageId(content.featured_image_id || '')
    setFeaturedImageUrl(content.featured_image?.url || '')
