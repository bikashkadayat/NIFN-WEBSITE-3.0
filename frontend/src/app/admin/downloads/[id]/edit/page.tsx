...
import { useDownload, useUpdateDownload, useDownloadCategories } from '@/hooks/use-queries'
...
export default function EditDownloadPage() {
  ...
  const { data: dl, isLoading } = useDownload(id)
  ...
  if (dl && !loaded) {
    const en = dl.translations?.find(t => t.locale === 'en')
    const ne = dl.translations?.find(t => t.locale === 'ne')
    setEnTitle(en?.title || '')
    setNeTitle(ne?.title || '')
    setEnDescription(en?.description || '')
    setNeDescription(ne?.description || '')
    setCategoryId(dl.category_id || '')
    setThumbnailId(dl.thumbnail_id || '')
    setSortOrder(dl.sort_order ?? 0)
    setIsActive(dl.is_active ?? true)
    setLoaded(true)
  }
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (f) {
      if (f.size > 50 * 1024 * 1024) { alert('File must be less than 50MB'); return }
      setFile(f)
    }
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">Edit Download</h1>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-lg font-semibold mb-4">File {dl?.file_name && <span className="text-sm font-normal text-gray-500">(Current: {dl.file_name} - {formatSize(dl.file_size)})</span>}</h2>
          ...
          >
            {file ? (
              ...
              <>
                ...
                <p className="text-sm text-gray-600">Click to replace file (leave unchanged to keep existing)</p>
              </>
            )}
        ...
        <div className="flex items-center gap-3">
          <button type="submit" disabled={updateMutation.isPending} className="px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 text-sm font-medium">
            {updateMutation.isPending ? 'Updating...' : 'Update Download'}
          </button>
