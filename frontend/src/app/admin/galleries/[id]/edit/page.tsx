...
import { useGallery, useUpdateGallery } from '@/hooks/use-queries'
...
export default function EditGalleryPage() {
  ...
  if (gallery && !loaded) {
    const en = gallery.translations?.find(t => t.locale === 'en')
    const ne = gallery.translations?.find(t => t.locale === 'ne')
    setTranslations({
      en: { title: en?.title || '', slug: en?.slug || '', description: en?.description || '' },
      ne: { title: ne?.title || '', slug: ne?.slug || '', description: ne?.description || '' },
    })
    setIsPublished(gallery.is_published ?? true)
    setSortOrder(gallery.sort_order ?? 0)
    setEventDate(gallery.event_date ? gallery.event_date.split('T')[0] : '')
    setLoaded(true)
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="mb-4">
            ...
            <input type="text" value={t.title} onChange={(e) => updateTranslation(activeLocale, 'title', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" required={activeLocale === 'en'} />
          </div>
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">Slug ({activeLocale})</label>
            <input type="text" value={t.slug} onChange={(e) => updateTranslation(activeLocale, 'slug', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" required />
          </div>
          <div className="mb-4">
        ...
        <div className="flex items-center gap-3">
          <button type="submit" disabled={updateMutation.isPending} className="px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 text-sm font-medium">
            {updateMutation.isPending ? 'Updating...' : 'Update Gallery'}
          </button>
