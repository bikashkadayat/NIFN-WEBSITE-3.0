...
export async function generateMetadata({ searchParams }: { searchParams: { locale?: string } }): Promise<Metadata> {
  const content = await fetchContent('privacy-policy', searchParams.locale)
...
export default async function PrivacyPolicyPage({ searchParams }: { searchParams: { locale?: string } }) {
  const content = await fetchContent('privacy-policy', searchParams.locale)
  if (!content || !content.body) return <ComingSoon title="Privacy Policy" />
