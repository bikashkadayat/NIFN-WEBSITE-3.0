...
export async function generateMetadata({ searchParams }: { searchParams: { locale?: string } }): Promise<Metadata> {
  const content = await fetchContent('terms', searchParams.locale)
...
export default async function TermsOfServicePage({ searchParams }: { searchParams: { locale?: string } }) {
  const content = await fetchContent('terms', searchParams.locale)
  if (!content || !content.body) return <ComingSoon title="Terms of Service" />
