...
import {
  ...
  useCreateDownloadCategory,
  useUpdateDownloadCategory,
  useDeleteDownloadCategory,
} from "@/hooks/use-download-categories"
import type { DownloadCategory, Locale } from "@/lib/types"
...
export default function DownloadCategoriesPage() {
  ...
  const createCategory = useCreateDownloadCategory()
  const updateCategory = useUpdateDownloadCategory()
  const deleteCategory = useDeleteDownloadCategory()

  const rawData = (categoriesQuery?.data as unknown as AnyRecord)?.data ?? categoriesQuery?.data
  const categories = Array.isArray(rawData) ? (rawData as DownloadCategory[]) : []
  ...
  const openEdit = (cat: DownloadCategory) => {
  ...
  const columns: Column<DownloadCategory>[] = [
    {
      key: "sno",
      label: "S.No",
    ...
    },
    {
      key: "downloads_count",
      label: "Downloads",
      render: (item) => <span className="text-zinc-600 dark:text-zinc-400">{item.downloads_count ?? 0}</span>,
    },
    {
      key: "is_active",
      label: "Status",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Download Categories</h1>
          <p className="mt-1 text-sm text-zinc-500">Manage download categories</p>
        </div>
      ...
      <Card>
        <div className="p-4">
          {categoriesQuery.isLoading ? (
            ...
            <EmptyState
              title="No download categories found"
              description="Create your first download category."
