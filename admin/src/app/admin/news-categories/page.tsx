...
} from "@/hooks/use-news-categories"
import type { NewsCategory, Locale } from "@/lib/types"
...
export default function NewsCategoriesPage() {
  ...
  const categories = Array.isArray(rawData) ? (rawData as NewsCategory[]) : []
  ...
  const openEdit = (cat: NewsCategory) => {
    ...
    setForm({
      translations: {
        en: { name: en.name ?? en.title ?? "", description: en.description ?? "" },
        ne: { name: ne.name ?? ne.title ?? "", description: ne.description ?? "" },
      },
      slug: cat.slug,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      translations: [
        { locale: "en", name: form.translations.en.name, description: form.translations.en.description },
        { locale: "ne", name: form.translations.ne.name, description: form.translations.ne.description },
      ],
    }
    if (editingId) {
  ...
  const columns: Column<NewsCategory>[] = [
    ...
    },
    {
      key: "news_count",
      label: "Articles",
      render: (item) => <span className="text-zinc-600 dark:text-zinc-400">{item.news_count ?? 0}</span>,
    },
    {
      key: "is_active",
      label: "Status",
  ...
  return (
    <div className="space-y-6">
      <Breadcrumb items={[{ label: "Dashboard", href: "/admin" }, { label: "News Categories" }]} />

      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">News Categories</h1>
          <p className="mt-1 text-sm text-zinc-500">Manage news categories</p>
        </div>
        ...
        <div className="p-4">
          {categoriesQuery.isLoading ? (
            ...
            <EmptyState
              ...
              description="Create your first news category."
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder={`Description in ${locale === "en" ? "English" : "Nepali"}`}
                    rows={2}
                  />
                </div>
              </div>
